...
def start_quiz(choice='all'):
    ...
    while len(question_list) > 0:
        print('\n' + s.col.Style.BRIGHT + s.col.Style.UNDERLINE + s.col.Fore.BLUE + str(i) + ': ', end='')
        ...
        while user_input > s.max_choices or user_input < 1:
            try:
                user_input = int(input('\n' + s.col.Style.BRIGHT + 'Choose an answer:' + s.col.Style.RESET_ALL))
            except:
    ...
    for x in s.answer_list:
        if not x['correct']:
            print(x['question'], s.col.Fore.RED + s.col.Style.BRIGHT + 'WRONG!' + s.col.Style.RESET_ALL, 'Correct answer:',
                  x['correct_answer'])
